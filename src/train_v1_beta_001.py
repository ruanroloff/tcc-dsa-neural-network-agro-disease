...
def model_fn(num_classes, image_size):
    ...
    model = Sequential([
        ...
        Dropout(0.24),
        Dense(48, activation=tf.keras.layers.LeakyReLU(alpha=0.2)),
        Dropout(0.24),
...
def main(args):
    ...
    callbacks = [
        EarlyStopping(min_delta=0.0001, patience=12, restore_best_weights=True),
        ModelCheckpoint(os.path.join(args.model_dir, 'best_model.h5'), save_best_only=True),
        ReduceLROnPlateau(monitor='val_loss', factor=0.22, patience=4, min_lr=0.001)
    ]
